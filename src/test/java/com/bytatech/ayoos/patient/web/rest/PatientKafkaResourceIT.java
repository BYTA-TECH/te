...
import com.bytatech.ayoos.patient.config.KafkaProperties;
...
class PatientKafkaResourceIT {
    ...
    void setup() {
        ...
        PatientKafkaResource kafkaResource = new PatientKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/patient-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/patient-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
